...
当前衍生修改包括实时布局的连续物品显示、白色物品分隔线、左右列表的可选显示列控制，以及绿/蓝/紫/金/红品质显示。后续个性化修改会在本仓库继续进行。

## 更新日志

### 2026-08-13

- 实时布局新增可选的 1x1 相邻加成预估：默认关闭，仅在悬浮提示中展示，不参与总分、排序或 Worker 求解评分。
- 品质显示统一为绿、蓝、紫、金、红；蕴金戒补充相邻攻击预估数值（绿 2%、蓝 4%、紫 6%、金 10%）。
...
产出的条目结构：`bonus`（加成类型/作用域）、`shape`（二维数组）、`value`（普通法宝 4 行品质 / 红法宝 1 行），以及可选的 `previewAdjacent`（实时布局相邻预估）。红法宝固定品质由 index.html 运行时派生，不在数据文件里。

## 布局目标与相邻预估

- 选择“填满优先”时，求解器先最大化占格数，再按攻/防/血权重最大化归一化属性分；“属性优先”则把占格作为次级目标。快速求解与混合最优均为限时启发式搜索，结果不保证数学上的全局最优。
- 自身加成参与求解评分。相邻加成只有在图鉴条目带 `p` 标记、形状为 1x1，且用户开启“相邻加成预估”时，才会在实时布局提示中折算到周围同系法宝的对应属性。
- 相邻预估默认关闭，不参与 Worker 评分、理论上界、总分或布局排序；后续新增同类法宝时，在法宝头行追加 `，p`，再重跑文本图鉴生成器。
...
- 法宝头行：`名称，形状代号[，作用域[，加成类型[，布局预估]]]`
  ...
  - 作用域：`z` 自身、`l` 相邻，**留空 = 无加成**；加成类型：`a` 攻击、`d` 防御、`h` 血量，留空默认 `a`；布局预估：`p` 仅用于 1x1 相邻加成的实时布局可选属性预估，不参与求解评分
