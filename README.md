...
- 品质显示统一为绿、蓝、紫、金、红；蕴金戒补充相邻攻击加成数值（绿 2%、蓝 4%、紫 6%、金 10%）。
...
- 自身加成参与求解评分。图鉴条目带 `p` 标记、形状为 1x1 的相邻加成，在用户开启“计入相邻加成”后会折算到四向接触的同系法宝对应基础属性（伤害加成为攻击力），并计入总属性、排序、Worker 评分和深度搜索；同一件相邻法宝即使多边接触也只计算一次，多个提供者的百分比相加。
