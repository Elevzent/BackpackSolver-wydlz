...
正式地址为 [http://wydlz.duckdns.org/BackpackSolver-wydlz/](http://wydlz.duckdns.org/BackpackSolver-wydlz/)，灰度地址为 [http://wydlz.duckdns.org/BackpackSolver-wydlz-gray/](http://wydlz.duckdns.org/BackpackSolver-wydlz-gray/)。灰度验证通过并经确认后才可更新正式站点。服务器连接、部署校验、服务与端口说明见总项目目录的 [../docs/server-runbook.md](../docs/server-runbook.md)。
...
- 新增独立灰度站点，正式目录保持不变；`blocks.data.js` 在入口页使用版本参数，避免静态缓存延迟数据更新。
...
首次打开截图导入时，页面按需加载本地 `lib/opencv.js`；部署环境应为该资源提供压缩传输和缓存。若安全策略修改 CSP，`script-src` 必须保留 `blob:`、`'wasm-unsafe-eval'` 和 `'unsafe-eval'`：OpenCV 的 embind 运行时会使用 `new Function(...)` 生成绑定代码，缺少任一例外都会导致组件下载后初始化失败。
